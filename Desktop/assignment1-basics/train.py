...
def main():
    ...
    # Create dummy data if data file is missing
    if not os.path.exists(args.data_path):
        print(f"Creating dummy data at {args.data_path}...")
    ...
    if len(train_ds) == 0:
        raise ValueError("Training dataset is empty. Increase data size or reduce context length.")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if batch_idx % 100 == 0:
                ...
                print(f"Step {batch_idx} | Memory: {mem_status}")
        ...
        print(f"[Epoch {epoch} Train Done] Time: {train_duration:.2f}s | "
              f"Avg Loss: {epoch_tloss:.4f} | Memory: {get_memory_usage(device)}")
        ...
        with torch.no_grad():
            ...
            print(f"[Epoch {epoch} Val Done] Time: {val_duration:.2f}s | "
                  f"Val Loss: {epoch_vloss:.4f} | Memory: {get_memory_usage(device)}")
