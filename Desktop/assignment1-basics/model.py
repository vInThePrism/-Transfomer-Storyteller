    ...
    def forward(self, x, mask):
        ...
        if self.flash:
            out = torch.nn.functional.scaled_dot_product_attention(
                ...
                attn_mask=None,
                dropout_p=0,
                is_causal=True  # auto-generates causal lower-triangular mask
            )
        else:
...
def generate_with_sampling(
    ...
    # Block special token IDs (0-4) from being generated
    ...
    for _ in range(max_new_tokens):
        ...
        # Clamp temperature to avoid division by near-zero
...
if __name__ == "__main__":
    ...
    if os.path.exists("ckpt/epoch_5.pt"):
        ...
        print(f"[INFO] Checkpoint loaded, device: {device}")
    else:
